...
public class DbSaveAssertUtility {
	...
	private static class ProductRevisionAsserter extends EntityAsserter<ProductRevision> {
		...
		public ProductRevision extractData(ResultSet rs) throws SQLException, DataAccessException {
			...
			if(_entity.getCategory() != null)
				assertThat(rs.getLong("r_category_id"), is(_entity.getCategory().getId())); // TODO assert null in db if category is null
	...
	public static void assertEntitySaved(Product product) {
		...
		String statment = "SELECT p.ent_id AS p_ent_id, " +
		"ent.ent_id AS ent_ent_id, " +
	...
	public static void assertEntitySaved(ProductRevision productRevision) {
		...
		String statment = "SELECT rev.ent_id AS r_ent_id, " +
		"rev.rev AS r_rev, " +
		"rev.unit AS r_unit, " +
		"rev.amount AS r_amount, " +
		"rev.category_id AS r_category_id, " +
		"rev.imageUrl AS r_imageUrl, " +
		"entRev.ent_id AS er_ent_id, " +
		"entRev.rev AS er_rev, " +
		"entRev.title AS er_title, " +
		"entRev.created_at AS er_created_at, " +
		"entRev.creator AS er_creator " +
		"FROM productRevision AS rev JOIN entityRevision AS entRev " +
		"ON rev.ent_id = entRev.ent_id AND rev.rev = entRev.rev " +
		"WHERE rev.ent_id = ? AND rev.rev = ?";
		DbSaveAssertUtility._jdbcOperations.query(statment, DbSaveAssertUtility._doubleId.setIds(productRevision.getId(), productRevision.getRevisionNumber()),
