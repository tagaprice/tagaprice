...
public class ReceiptDAO implements IReceiptDAO {
	...
	@Override
	public ReceiptData save(ReceiptData receipt) throws DAOException {
		_log.debug("receipt:"+receipt);

		ReceiptData versionedReceipt = _entityDAO.save(receipt);
		...
		try {
			if(versionedReceipt.getRev()==1){
				...
				//Add new
				for(ProductData pd:versionedReceipt.getProductData()){
