...
public class SearchServiceImpl extends RemoteServiceServlet implements ISearchService {
	...
	@Override
	public List<StatisticResult> searchProductPrices(String productId, BoundingBox bbox, Date begin, Date end) throws DaoException {
		return statisticDao.searchPricesViaProduct(productId, bbox, begin, end);
	}

	@Override
	public List<StatisticResult> searchShopPrices(String shopId, Date begin, Date end) throws DaoException {
	...
	public List<StatisticResult> searchShopCategoryPrices(String shopCategoryId, BoundingBox bbox, Date begin, Date end)throws DaoException {
		List<StatisticResult> results = statisticDao.searchPricesViaShopCategory(shopCategoryId, bbox, begin, end);
		
		for(Shop s: shopDAO.listByCategory(shopCategoryId, bbox)){
			results.add(new StatisticResult(new Date(s.getLastModified()), s, null, null, null));
		}
