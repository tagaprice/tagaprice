...
public class TaPManager {
	...
	private Address address = new Address("Flossgasse", "vienna", new Country(
			"at", "Austria", "Österreich"), 48.21960, 16.37205);
		...
		History.addValueChangeHandler(new ValueChangeHandler<String>() {
			...
			public void onValueChange(ValueChangeEvent<String> event) {
				...
				if (historyToken[0].equals("receipt/get")) {
					...
					TaPMng.showReceiptPage(new Receipt(Long
							.parseLong(equalToken[1])));
					...
					if (historyToken.length > 1) {
						String[] equalToken = historyToken[1].split("=");
						if (equalToken[0].equals("confirm")
								&& !equalToken[1].isEmpty()) {
	...
	public void showProductPage(final Long id) {
		...
		try {
			RPCHandlerManager.getProductHandler().get(id,
					new AsyncCallback<Product>() {
						...
						@Override
						public void onFailure(Throwable caught) {
							uiMng.showInfo("Fail product: " + caught,
									BoxType.WARNINGBOX);
	...
	public void newProductPage(String title) {
		...
		RPCHandlerManager.getTypeHandler().get(null,
				new AsyncCallback<Category>() {
					...
					public void onSuccess(Category result) {
						Product pd3 = new Product("Defaulf Procut Title", 1,
								1l, 2l, result.getId(), "logo.png", null);
			...
			public void onSuccess(final Shop result) {
				RPCHandlerManager.getTypeHandler().get(
						...
						new AsyncCallback<Category>() {
							...
							@Override
							public void onFailure(Throwable caught) {
								uiMng.showInfo("Fail type: " + caught,
										BoxType.WARNINGBOX);
							}
						});
	...
	public void newShopPage(final String title) {
		...
		RPCHandlerManager.getTypeHandler().get(null,
				new AsyncCallback<Category>() {
					...
					@Override
					public void onFailure(Throwable caught) {
						uiMng.showInfo("Fail shop: " + caught,
								BoxType.WARNINGBOX);
					}

					@Override
					public void onSuccess(Category result) {
						Shop sd = new Shop("Default Shop Title", 1, 1l, null,
								"logo.png", TaPMng.getCurrentAddress());
	...
	public void showReceiptPage(Receipt data) {
		...
		try {
			RPCHandlerManager.getReceiptHandler().get(data,
	...
	/**
	 * Returns the current Address and Position. Is is possible to set a static
	 * Address.
	 * 
	 * @return the current Address and Position
	 */
	...
	public void refeshCurrentAdress() {
		...
		if (this.currentLocation == null) {
			this.currentLocation = Geolocation.getGeolocation();
		}

		this.currentLocation.getCurrentPosition(new PositionCallback() {

			@Override
			public void onSuccess(Position position) {
				TaPManager.this.address.setCoordinates(position.getCoords()
