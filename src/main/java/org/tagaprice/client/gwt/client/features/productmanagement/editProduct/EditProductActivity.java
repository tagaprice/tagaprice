...
public class EditProductActivity implements EditProductView.Presenter, Activity {
	private static final MyLogger logger = LoggerFactory.getLogger(EditProductActivity.class);
	 ...
	 * @param clientFactory
	 */
	public EditProductActivity(EditProductPlace place, ClientFactory clientFactory) {
		EditProductActivity.logger.log("EditProductActivity created");
		EditProductActivity.logger.log("Product ID: " + String.valueOf(place.getProductId()));
		this.place = place;
		...
		if (this.place.isNewProduct()) {
			...
			this.clientFactory.getProductServiceDispatch().getProductById(this.place.getProductId(),
				...
				public void onFailure(Throwable caught) {
					...
					// TODO Auto-generated method stub
					EditProductActivity.logger.log("UNSUCCESFULL call");
				}
			});
		}
	...
	public void updateView(Product p) {
		EditProductView editProductView = this.clientFactory.getEditProductView();
	...
	public void onSaveButtonClicked(ClickEvent event) {
		EditProductActivity.logger.log("Save Button clicked");
		EditProductView editProductView = this.clientFactory.getEditProductView();

		Product p = new ProductImpl(editProductView.getId(), editProductView.getName(), editProductView.getPrice(),
		...
		this.clientFactory.getProductServiceDispatch().saveProduct(p, !this.place.isNewProduct(),
