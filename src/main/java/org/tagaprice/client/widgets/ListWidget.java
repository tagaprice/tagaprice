...
public class ListWidget<T extends APagePreview> extends Composite {
	...
	public Entity getSelection() {
		...
		APagePreview ep = (APagePreview) verticalPanel
				.getWidget(currentSelection);
	...
	public APagePreview getSelectionPreview() {
		APagePreview rc = (APagePreview) verticalPanel
				.getWidget(currentSelection);
		currentSelection = -1;
		return rc;
	}
	...
	public void highlightNextSuggestion() {
		if (currentSelection != -1)
			verticalPanel.getWidget(currentSelection).removeStyleName(
					"highlightSuggestion");
		if (currentSelection < verticalPanel.getWidgetCount() - 1)
			currentSelection++;
		verticalPanel.getWidget(currentSelection).addStyleName(
				"highlightSuggestion");
